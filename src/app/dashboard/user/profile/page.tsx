  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/user/profile", {
        ...
        body: JSON.stringify({ name, headline, location, skills, avatar }),
      });
    ...
    <>
      <header className="flex items-center justify-between mb-10">
        <div>
          ...
          <p className="text-gray-400 text-sm">Update your profile to attract employers.</p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
          className="bg-neon-green hover:bg-[#3cd612] text-background-dark font-bold text-sm px-6 py-2.5 rounded-full transition-all disabled:opacity-50"
        >
          ...
          <div className="glass-panel p-8 rounded-2xl">
            ...
            <div className="flex flex-col md:flex-row gap-8">
              ...
              <div className="flex-1 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase">Headline</label>
                  <input
                    type="text"
                    value={headline}
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase">Location</label>
                  <input
                    type="text"
                    value={location}
          ...
          <div className="glass-panel p-8 rounded-2xl">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {skills.map((skill) => (
                <span
                  key={skill}
                  className="px-3 py-1.5 bg-neon-green/10 text-neon-green rounded-full text-sm font-medium flex items-center gap-2"
                >
                  {skill}
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-xl bg-card-dark border border-white/10 text-white focus:border-neon-green focus:outline-none"
              />
              <button
                onClick={addSkill}
          ...
          <div className="glass-panel p-6 rounded-2xl sticky top-6">
            ...
            <ul className="space-y-3 text-sm text-gray-400">
              <li className="flex items-start gap-2">
                <span className="material-symbols-outlined text-neon-green text-sm mt-0.5">check_circle</span>
              ...
              <li className="flex items-start gap-2">
                <span className="material-symbols-outlined text-gray-600 text-sm mt-0.5">radio_button_unchecked</span>
                Add work experience
              </li>
            </ul>
          </div>
