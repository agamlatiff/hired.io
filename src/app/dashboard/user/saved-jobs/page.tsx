...
interface SavedJob { id: string; savedAt: string; job: { id: string; roles: string; location: string | null; jobType: string; salaryFrom: string; salaryTo: string; currency: string; Company: { id: string; name: string; logo: string | null } | null } }
...
export default function SavedJobsPage() {
  ...
  useEffect(() => {
    async function fetchSavedJobs() {
      try { const res = await fetch("/api/user/saved-jobs"); if (res.ok) setSavedJobs(await res.json()); }
      catch (error) { console.error("Error:", error); }
      finally { setLoading(false); }
    }
    if (session) fetchSavedJobs();
  }, [session]);
  ...
  return (
    <>
      <header className="flex items-center justify-between mb-10">
        <div><h2 className="text-3xl font-bold text-white mb-1">Saved Jobs</h2><p className="text-gray-400 text-sm">Jobs you&apos;ve bookmarked for later.</p></div>
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 rounded-2xl text-center">
          ...
          <p className="text-gray-400 mb-6">Save jobs while browsing.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {savedJobs.map((saved) => (
            <div key={saved.id} className="glass-panel p-6 rounded-2xl flex flex-col group hover:border-neon-green/50 transition-all">
              ...
              <div className="flex flex-wrap gap-2 mb-4">{saved.job.location && <span className="px-2 py-1 bg-card-dark rounded text-xs text-gray-300">{saved.job.location}</span>}<span className="px-2 py-1 bg-card-dark rounded text-xs text-gray-300">{saved.job.jobType}</span></div>
              <p className="text-sm text-neon-green font-bold mb-4">{saved.job.currency} {parseInt(saved.job.salaryFrom).toLocaleString()} - {parseInt(saved.job.salaryTo).toLocaleString()}</p>
              <div className="mt-auto flex gap-2">
                <Link href={`/detail/job/${saved.job.id}`} className="flex-1 text-center px-4 py-2 bg-white/10 rounded-xl text-sm font-medium hover:bg-white/20">View</Link>
                <Link href={`/detail/job/${saved.job.id}#apply`} className="flex-1 text-center px-4 py-2 bg-neon-green text-background-dark rounded-xl text-sm font-bold hover:bg-[#3cd612]">Apply</Link>
              </div>
            </div>
          ))}
        </div>
