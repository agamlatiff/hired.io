...
interface Conversation { id: string; company: { id: string; name: string; logo: string | null }; messages: { content: string; createdAt: string }[]; _count: { messages: number } }
...
export default function UserMessagesPage() {
  ...
  useEffect(() => {
    async function fetchConversations() {
      try { const res = await fetch("/api/user/messages"); if (res.ok) setConversations(await res.json()); }
  ...
  useEffect(() => {
    async function fetchMessages() {
      ...
      try {
        const res = await fetch(`/api/user/messages/${selectedId}`);
          ...
          {loading ? <div className="p-4 space-y-3">{[1, 2, 3].map((i) => <div key={i} className="h-16 bg-gray-700 rounded-xl animate-pulse" />)}</div> : conversations.length === 0 ? (
            ...
            conversations.map((conv) => (
              <button key={conv.id} onClick={() => setSelectedId(conv.id)} className={`w-full p-4 flex items-center gap-3 hover:bg-white/5 transition-colors ${selectedId === conv.id ? "bg-white/10" : ""}`}>
                <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center overflow-hidden">{conv.company.logo ? <img src={conv.company.logo} alt="" className="w-full h-full object-contain" /> : <span className="text-gray-800 font-bold">{conv.company.name.charAt(0)}</span>}</div>
                <div className="flex-1 text-left overflow-hidden">
                  <p className="font-bold text-white text-sm truncate">{conv.company.name}</p>
        ...
        {!selectedId ? (
          ...
          <>
            <div className="p-4 border-b border-white/10 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center overflow-hidden">{selectedConversation?.company.logo ? <img src={selectedConversation.company.logo} alt="" className="w-full h-full object-contain" /> : <span className="text-gray-800 font-bold">{selectedConversation?.company.name.charAt(0)}</span>}</div>
              <div><p className="font-bold text-white">{selectedConversation?.company.name}</p></div>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.senderType === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[70%] px-4 py-3 rounded-2xl ${msg.senderType === "user" ? "bg-neon-green text-background-dark rounded-br-none" : "bg-card-dark text-white rounded-bl-none"}`}>
                    <p className="text-sm">{msg.content}</p>
                    <p className={`text-xs mt-1 ${msg.senderType === "user" ? "text-background-dark/60" : "text-gray-500"}`}>{new Date(msg.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                </div>
              ))}
