...
interface Interview {
  ...
  applicant: {
    ...
    Job: { id: string; roles: string } | null;
  };
}
...
export default function InterviewsPage() {
  ...
  const updateStatus = async (interviewId: string, status: string) => {
    try {
      ...
      if (res.ok) {
        setInterviews(interviews.map((i) => (i.id === interviewId ? { ...i, status } : i)));
  ...
  return (
    <>
      <header className="flex items-center justify-between mb-10">
        <div>
          <h2 className="text-3xl font-bold text-white mb-1">Interview Schedule</h2>
          <p className="text-gray-400 text-sm">Manage your scheduled interviews with candidates.</p>
        </div>
      </header>
      ...
      <div className="flex gap-2 mb-6">
        {["upcoming", "past", "cancelled", "all"].map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${filter === f ? "bg-neon-green text-background-dark" : "bg-card-dark text-gray-300 hover:bg-white/10"
              }`}
          >
      ...
      {loading ? (
        <div className="space-y-4">{[1, 2, 3].map((i) => <div key={i} className="h-24 bg-gray-700 rounded-xl animate-pulse" />)}</div>
        ...
        <div className="glass-panel p-12 rounded-2xl text-center">
          ...
          <p className="text-gray-400">Schedule interviews from the applicants page.</p>
        </div>
      ) : (
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl bg-neon-purple/20 flex items-center justify-center">
                    <span className="material-symbols-outlined text-neon-purple">{getTypeIcon(interview.type)}</span>
                  </div>
                  <div>
                    <h3 className="font-bold text-white">{interview.applicant.user?.name || "Unknown"}</h3>
                    <p className="text-sm text-gray-400">{interview.applicant.Job?.roles || "Position"}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-6 text-sm">
                  ...
                  <div className="text-center">
                    ...
                    <p className="font-bold text-white">{new Date(interview.scheduledAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                  <div className="text-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {interview.location && (
                    <a href={interview.location} target="_blank" className="px-3 py-1.5 bg-blue-500/20 text-blue-400 rounded-lg text-xs font-bold hover:bg-blue-500/30">
                      Join
                    </a>
                  )}
                </div>
