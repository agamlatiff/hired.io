...
export async function POST(request: Request) {
  try {
    ...
    const savedJob = await prisma.savedJob.create({ data: { userId: user.id, jobId } });
    return NextResponse.json(savedJob, { status: 201 });
  } catch (error: unknown) {
    if ((error as { code?: string }).code === "P2002") return NextResponse.json({ error: "Already saved" }, { status: 409 });
    console.error("Error:", error);
    return NextResponse.json({ error: "Failed" }, { status: 500 });
  }
}
